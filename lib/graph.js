...
function createGraph(modules, circular, config, options) {
	...
	// hypothetically this ends up being something like ["src/some-module"]
	const rootPaths = config.path.map((path) => path.split('/').slice(0, 2).join('/'));

	Object.keys(modules).forEach((id) => {
		// don't carry on through domain boundaries else the graph will get too big
		if (config.noDomainCrossing && !rootPaths.some((path) => id === path || id.startsWith(`${path}/`))) {
			return;
		}
		...
		modules[id].forEach((depId) => {
			...
			// color boundary violations to make them stand out
			if (config.noDomainCrossing && !rootPaths.some((path) => depId.startsWith(path))) {
				setNodeColor(nodes[depId], config.boundaryViolationColor);
			}
